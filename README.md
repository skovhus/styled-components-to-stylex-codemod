...
When styled-components reference other components as selectors across file boundaries (e.g., `` styled.div`${Icon} { color: red }` `` where `Icon` is imported from another file), the codemod needs to scan consumer files to wire up the cross-file overrides.

Use `consumerPaths` to specify additional files to scan:
...
- **Files in both `files` and `consumerPaths`** use the marker strategy (both sides are transformed).
- **Files only in `consumerPaths`** trigger the bridge strategy (stable className + toString shim for incremental migration where consumers aren't yet transformed).

> [!NOTE]
> `consumerPaths` requires the optional dependency `oxc-resolver` for import resolution.
> Install it when using this option:
>
> ```bash
> npm install oxc-resolver
> # or
