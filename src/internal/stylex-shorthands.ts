 ...
 */
const PHYSICAL_ONLY_SHORTHANDS = new Set<string>(["scrollMargin", "scrollPadding"]);
...
export function splitDirectionalProperty(args: {
  ...
  if (values.length === 1 && !important && !alwaysExpand && !physicalOnly) {
    return [{ prop, value: withImportant(top) }];
  }

  const quad = expandQuadValues(values);
  if (important || physicalOnly) {
    return [
