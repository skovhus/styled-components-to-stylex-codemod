...
export type ScopeHelpers = {
  getRootIdentifierInfo: typeof extractRootAndPath;
  resolveImportInScope: (
    localName: string,
    identNode?: unknown,
  ) => { importedName: string; source: ImportSource } | null;
  resolveImportForExpr: (
    expr: unknown,
    localName: string,
  ) => { importedName: string; source: ImportSource } | null;
  buildSafeIndexedParamName: (preferred: string, containerExpr: ExpressionKind | null) => string;
};

export function createScopeHelpers(args: {
  ...
  return {
    getRootIdentifierInfo: extractRootAndPath,
    resolveImportInScope,
    resolveImportForExpr,
