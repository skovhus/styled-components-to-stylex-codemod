...
describe("theme boolean conditionals", () => {
  it("should transform theme.isDark conditional with props param pattern", () => {
    const source = `
import styled from "styled-components";
...
`;
    ...
    const result = transformWithWarnings(
      { source, path: "theme-isdark-props.tsx" },
      { jscodeshift: j, j, stats: () => {}, report: () => {} },
      { adapter: fixtureAdapter },
    );
    ...
    expect(result.code).toMatchInlineSnapshot(`
      ...
      function Box(props: React.PropsWithChildren<{ ref?: React.Ref<HTMLDivElement> }>) {
        ...
        return (
          <div
            {...stylex.props(styles.box, theme.isDark ? styles.boxDark : styles.boxLight)}>{children}</div>
        );
      }
  ...
  it("should transform theme.isDark conditional with destructured theme pattern", () => {
    const source = `
import styled from "styled-components";
...
`;
    ...
    const result = transformWithWarnings(
      { source, path: "theme-isdark-destructured.tsx" },
      { jscodeshift: j, j, stats: () => {}, report: () => {} },
      { adapter: fixtureAdapter },
    );
...
`;
  ...
  it("should accumulate multiple theme boolean properties into style buckets", () => {
    const source = `
import styled from "styled-components";
...
const Box = styled.div\`
  padding: 16px;
  mix-blend-mode: \${(props) => props.theme.isDark ? "lighten" : "darken"};
  background-color: \${({ theme }) => theme.isDark ? "#111" : "#fff"};
\`;
...
`;
    ...
    const result = transformWithWarnings(
      { source, path: "theme-isdark-multiple.tsx" },
      { jscodeshift: j, j, stats: () => {}, report: () => {} },
      { adapter: fixtureAdapter },
    );
...
`;
  ...
  it("should warn with 'Theme-dependant call expression' when ternary branches contain theme helper calls", () => {
    const source = `
import styled from "styled-components";
...
`;
    ...
    const result = transformWithWarnings(
      { source, path: "theme-call-expr.tsx" },
      { jscodeshift: j, j, stats: () => {}, report: () => {} },
      { adapter: fixtureAdapter },
    );
    ...
    expect(result.warnings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "Theme-dependant call expression could not be resolved (e.g. theme helper calls like theme.highlight() are not supported)",
        }),
      ]),
    );
  });

  it("should warn with 'Theme-dependent block-level conditional' when css block branches contain dynamic interpolations", () => {
    const source = `
import styled from "styled-components";

const Box = styled.div\`
  \${(props) => props.theme.isDark
    ? \`color: \${props.someDynamic};\`
    : "color: black;"}
\`;
...
`;
    ...
    const result = transformWithWarnings(
      { source, path: "theme-block-dynamic.tsx" },
      { jscodeshift: j, j, stats: () => {}, report: () => {} },
      { adapter: fixtureAdapter },
    );
  ...
  it("should warn with 'Theme value with fallback' for logical expressions using props.theme with ??/||", () => {
    const source = `
import styled from "styled-components";
...
`;
    ...
    const result = transformWithWarnings(
      { source, path: "theme-logical-fallback.tsx" },
      { jscodeshift: j, j, stats: () => {}, report: () => {} },
      { adapter: fixtureAdapter },
    );
    ...
    expect(result.warnings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: "Theme value with fallback (props.theme.X ?? / || default) cannot be resolved statically — use adapter.resolveValue to map theme paths to StyleX tokens",
        }),
      ]),
    );
  });
